...
//creating an array with Short title names
const shortMoviesTitle = movies.filter((movie) => {
  return movie.title.length <= 5;
});

//creating an array with long title names
const longMoviesTitle = movies.filter((movie) => {
  return movie.title.length > 6;
});

//counting the number of movies done between the 1980 and 1989
...
// creating an array with extra key(tag) based on the rating of the movies
const moviesWithTag = movies.map((movie) => {
  if (movie.rating >= 7) {
    movie["tag"] = "Good";
  } else if (movie.rating > 04 && movie.rating < 7) {
    movie["tag"] = "Average";
...
//chainig contain the movies rated higher than 6 and map the movies array to only the rating of the movies
